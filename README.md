...
Клонируйте репозиторий `side-checker` на ваш локальный компьютер:

```bash
git clone https://github.com/cyberomanov/side-checker.git && \
cd side-checker
```
...
```bash
tmux new -s side-checker; tmux attach -t side-checker
```
